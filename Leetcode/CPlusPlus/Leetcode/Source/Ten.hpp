...
*/
class NumUniqueEmails : public Solution
{
public:
    int numUniqueEmails(vector<string>& emails) {
        set<string> filtered;

        for (auto it = emails.cbegin(); it != emails.cend(); it++)
        {
            auto pos1 = it->find('@');
            auto domain = it->substr(pos1);
            auto local = it->substr(0, pos1);

            auto pos2 = local.find('+');
            if (pos2 != string::npos)
            {
            ...
            {
                local = local.erase(local.find('.'), 1);
            }

            filtered.insert(local + domain);
        }
